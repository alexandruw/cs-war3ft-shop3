...
public WC3_ResetGame()
{
	...
	for ( i=0; i < numplayers; i++ )
	{
		...
		g_bPlayerAlreadyBoughtSlap[id] = false;
		g_bPlayerAlreadyBoughtOneHit[id] = false;
...
WC3_ShowBar( id )
{
	...
	{
		new iSlot=0;
		if (ITEM_isSlotFilled(id,ITEM_SLOT_ONE))
			iSlot = ITEM_SLOT_ONE;
		if (ITEM_isSlotFilled(id,ITEM_SLOT_TWO))
			iSlot = ITEM_SLOT_TWO;
		if (ITEM_isSlotFilled(id,ITEM_SLOT_THREE))
			iSlot = ITEM_SLOT_THREE;
		ITEM_Format( id, g_iShopMenuItems[id][iSlot], szItemName, 31 )
		pos += formatex( szItemInfo[pos], 256-pos, "%s", szItemName );
	}
	else if (iItemCount==2)
	{
		//find second item slot
		new firstItemSlot=-1;
		new secondItemSlot=-1;
		
		if (ITEM_isSlotFilled(id,ITEM_SLOT_ONE))
			firstItemSlot = ITEM_SLOT_ONE;
		else if (ITEM_isSlotFilled(id,ITEM_SLOT_TWO))
			firstItemSlot = ITEM_SLOT_TWO;
			
		//we know first item slot by now
		if (ITEM_isSlotFilled(id,ITEM_SLOT_THREE))
			secondItemSlot=ITEM_SLOT_THREE;
		else if (ITEM_isSlotFilled(id,ITEM_SLOT_TWO))
			secondItemSlot=ITEM_SLOT_TWO;
		
			ITEM_Format( id, g_iShopMenuItems[id][firstItemSlot], szItemName, 31 )
			pos += formatex( szItemInfo[pos], 256-pos, "%s", szItemName );
			ITEM_Format( id, g_iShopMenuItems[id][secondItemSlot], szItemName, 31 )
			pos += formatex( szItemInfo[pos], 256-pos, " %L %s", id, "WORD_AND", szItemName );
	}
	else if (iItemCount==3)
	{
